...
def forwardconv(input, convparams):
    ...
    device=input.device
    convparams = convparams.to(device)
    if len(input.size()) == 4:
        ...
        scal = torch.stack([convparams[:, :, :, 1], convparams[:, :, :, 1], 
					convparams[:, :, :, 1]])
        scal = scal.permute(1,0,2,3).to(device)
		# preparing the input to fit the tensor multiplication
        input = input.permute(1, 0, 2, 3).to(device)
	...
	# and need to loop over the according outputs as well
        ...
        scal = scal.permute(1,0,2,3).to(device)
		# preparing the input to fit the tensor multiplication
        input = input.permute(2, 0, 1, 3, 4).to(device)
...
def quatmaxpool(input, size, stride=1):
    device = input.device
    output = torch.zeros(len(input[:,0,0,0,0]), len(input[0,:,0,0,0]), 3, 
                         math.floor((len(input[0,0,0,:,0]) - size) / stride) + 1, 
                         math.floor((len(input[0,0,0,:,0]) - size) / stride) + 1).to(device)
    amplitudes = torch.zeros(size, size).to(device)
        ...
        for c in range(len(input[0,:,0,0,0])):
            for i in range(math.floor((len(input[0,0,0,:,0]) - size) / stride) + 1):
                for j in range(math.floor((len(input[0,0,0,:,0]) - size) / stride) + 1):
                    ...
                    # Get the index of the maximum amplitude
                    maxcount = torch.argmax(amplitudes)
